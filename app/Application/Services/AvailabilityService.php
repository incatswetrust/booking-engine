 ...
 * buffer_before/after. §24: a resource with capacity > 1 stays bookable
 * for a slot as long as the sum of party_size across every overlapping
 * booking/hold plus the candidate's own party_size doesn't exceed
 * resource.capacity — resource_blocks and synced external-calendar busy
 * periods (§37) are different: those always fully block the resource
 * regardless of capacity, since they don't represent "capacity used by a
 * booking", they represent the resource being unavailable at all.
 *
...
class AvailabilityService
{
    ...
    public function forService(
        ...
        foreach ($resources as $resource) {
            $days = $this->cache->remember(
                ...
                $timezone,
                $partySize,
                function () use ($resource, $service, $dateFrom, $dateTo, $timezone, $partySize) {
                    $startedAt = microtime(true);
                    $result = $this->computeForResource($resource, $service, $dateFrom, $dateTo, $timezone, $partySize);
     ...
     */
    private function computeForResource(Resource $resource, Service $service, CarbonImmutable $dateFrom, CarbonImmutable $dateTo, string $timezone, int $partySize): array
    {
        ...
        $bookings = Booking::query()
            ...
            ->get(['start_at', 'end_at', 'service_id', 'party_size']);
        ...
        // Fully blocking regardless of capacity (§24) — a block or an
        // external-calendar busy period means the resource itself is
        // unavailable, not "some of its capacity is used".
        $fullyBlocked = $blocks->map(fn ($b) => [$b->starts_at, $b->ends_at])
        ...
        // computations. Each range carries its own party_size so
        // slotsWithinInterval can sum concurrent usage against the
        // resource's capacity instead of a single overlap fully blocking.
        $partyOccupancy = $bookings->map(fn ($b) => [
        ...
        for ($cursor = $dateFrom->setTimezone($tz)->startOfDay(); $cursor->lte($dateTo); $cursor = $cursor->addDay()) {
            ...
            foreach ($intervals as [$startTime, $endTime]) {
                $slots = $this->slotsWithinInterval(
                    $resource, $service, $dateKey, $startTime, $endTime, $tz, $fullyBlocked, $partyOccupancy, $partySize, $minStart,
                );
    ...
    /**
     * @param  Collection<int, array{0: CarbonInterface, 1: CarbonInterface}>  $fullyBlocked
     * @param  Collection<int, array{0: CarbonInterface, 1: CarbonInterface, 2: int}>  $partyOccupancy
    ...
    private function slotsWithinInterval(
        ...
        string $endTime,
        DateTimeZone $tz,
        Collection $fullyBlocked,
        Collection $partyOccupancy,
        int $partySize,
        ...
        while ($slotStart->addMinutes($service->duration_minutes)->lte($intervalEnd)) {
            ...
            if ($slotStart->gte($minStart)) {
                $overlaps = fn (array $range) => $range[0]->lt($slotEnd) && $range[1]->gt($slotStart);

                $blocked = $fullyBlocked->contains($overlaps);

                if (! $blocked) {
                    $bookedCapacity = $partyOccupancy->filter($overlaps)->sum(fn ($range) => $range[2]);
                    $blocked = ($bookedCapacity + $partySize) > $resource->capacity;
                }
